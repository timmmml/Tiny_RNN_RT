...
def _MB_step_core_variables(alpha, p_transit, c, s, o, Q_s, Q_mb):
    ...
    Q_s_new[ns] = (1. - alpha) * Q_s[ns] - alpha * o
...
class Model_based_symm(TwoStepModelCoreCSO):
    def __init__(self, p_transit=0.8, equal_reward=False):
        super().__init__()
    ...
    def _session_likelihood_core(self, session, params, DVs):
        ...
        DVs['trial_log_likelihood'], DVs['Q_s'], DVs['Q_mb'], DVs['scores'], DVs['choice_probs'] = _MB_session_likelihood_core(
            alpha, iTemp, self.p_transit, session['choices'], session['second_steps'], outcomes,
