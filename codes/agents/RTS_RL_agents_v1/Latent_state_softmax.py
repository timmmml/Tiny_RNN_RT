...
@jit(nopython=True)
def _LS_step_core_variables(p_r, good_prob, c, s, o, p_1):
...
@jit(nopython=True)
def _LS_session_likelihood_core(p_r, iTemp, good_prob, choices, second_steps, outcomes, p_1, scores, choice_probs, n_trials):
    ...
    for trial in range(n_trials):
        ...
        p_1[trial + 1] = _LS_step_core_variables(p_r, good_prob, c, s, o, p_1[trial])
...
class Latent_state_softmax(TwoStepModelCoreCSO):
    ...
    def _step_core_variables(self, trial_event, params):
        (c, s, o) = trial_event
        p_r, iTemp = params
        self.wm['p_1'] = _LS_step_core_variables(p_r, self.good_prob, c, s, o, self.wm['p_1'])
    ...
    def _session_likelihood_core(self, session, params, DVs):
        ...
        DVs['trial_log_likelihood'], DVs['p_1'], DVs['scores'], DVs['choice_probs'] = _LS_session_likelihood_core(
            p_r, iTemp, self.good_prob, session['choices'], session['second_steps'], session['outcomes'],
