...
class MF_moMF_MB_dec(RL_agent):
    '''Mixture agent with forgetting and motor level model free, seperate learning rates
    for motor and choice level model free.'''

    def __init__(self, kernels = ['bs', 'ck', 'rb']):
        self.name = 'MF_moMF_MB_dec'
        self.param_names  = ['alpQ', 'decQ','alpP', 'decP', 'lbd' , 'alpT', 'decT',
                             'G_td', 'G_tdm', 'G_mb']
        self.param_ranges = ['unit']*7 + ['pos']*3
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        # Unpack parameters.
        alpQ, decQ, alpP, decP, lbd, alpT, decT, G_td, G_tdm, G_mb = params_T[:10]
        ...
        if get_DVs: return self.get_DVs(session, params_T, Q_net, Q, M, P) | {'session_log_likelihood': session_log_likelihood(choices, Q_net), 'scores': Q_net}
