...
class MF_dec(RL_agent):
    'Model-free agent with forgetting.'
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        # Unpack parameters.
        alp, iTemp, lbd, dec = params_T[:4]   # Q value decay parameter.
        ...
        for i, (c, s, o) in enumerate(zip(choices[:-1], second_steps, outcomes)): # loop over trials.
            ...
            V[r,i+1] = V[r,i] * (1.-dec) # Second step forgetting.
        ...
        if get_DVs: return self.get_DVs(session, params_T, Q_net, Q) | {'session_log_likelihood': session_log_likelihood(choices, Q_net, iTemp), 'scores': Q_net * iTemp}
