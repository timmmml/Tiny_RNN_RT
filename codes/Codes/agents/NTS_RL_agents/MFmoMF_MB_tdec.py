...
class MFmoMF_MB_tdec(RL_agent):
    '''Mixture agent with transition prob. forgetting and motor level model free.'''

    def __init__(self, kernels = ['bs', 'ck', 'rb']):
        self.name = 'MFmoMF_MB_tdec'
        self.param_names  = ['alpQ', 'lbd','alpT', 'decT', 'G_td','G_tdm','G_mb']
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        # Unpack parameters.
        alpQ, lbd, alpT, decT, G_td, G_tdm, G_mb = params_T[:7]
