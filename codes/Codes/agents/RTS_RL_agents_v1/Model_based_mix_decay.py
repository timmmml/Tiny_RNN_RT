...
@jit(nopython=True)
def _MB_step_core_variables(alpha1, alpha2, b, p_transit, c, s, o, Q_s, Q_mb):
    ...
    if o == 1:
        Q_s_new[s] = alpha1 * Q_s[s] + 1
        Q_s_new[ns] = Q_s[ns] - b
    else:
        Q_s_new[s] = Q_s[s] + alpha2 * (Q_s[ns] - Q_s[s])
        Q_s_new[ns] = Q_s[ns]
...
@jit(nopython=True)
def _MB_session_likelihood_core(alpha1, alpha2, b, iTemp, p_transit, choices, second_steps, outcomes, Q_s, Q_mb, scores, choice_probs, n_trials):
    ...
    for trial in range(n_trials):
        ...
        Q_s[trial + 1], Q_mb[trial + 1] = _MB_step_core_variables(alpha1, alpha2, b, p_transit, c, s, o, Q_s[trial], Q_mb[trial])
...
class Model_based_mix_decay(TwoStepModelCoreCSO):
    def __init__(self, p_transit=0.8, b=None):
        super().__init__()
    ...
    def _step_core_variables(self, trial_event, params):
        ...
        if self.n_params == 4:
            ...
            b = self.fixed_b
        self.wm['Q_s'], self.wm['Q_mb'] = _MB_step_core_variables(alpha1, alpha2, b, self.p_transit, c, s, o, self.wm['Q_s'], self.wm['Q_mb'])
    ...
    def _session_likelihood_core(self, session, params, DVs):
        ...
        DVs['trial_log_likelihood'], DVs['Q_s'], DVs['Q_mb'], DVs['scores'], DVs['choice_probs'] = _MB_session_likelihood_core(
            alpha1, alpha2, b, iTemp, self.p_transit, session['choices'], session['second_steps'], session['outcomes'],
