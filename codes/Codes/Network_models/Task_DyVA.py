...
class TaskDyVA(BaseNNAgent):
    ...
    def __init__(self, model_params):
        ...
        # x, z, w dimensions (action modality space, latent space, and stochastic variable space)
        ...
        self.theta_0_hidden_size = model_params.get('theta0_hidden_size', 64)
        self.theta_0_hidden_actvation = model_params.get('theta0_hidden_activation', 'relu')
        self.theta_0_output_size = self.z_dim
        self.theta_0_output_activation = model_params.get('theta0_output_activation', 'linear')

        self.f_theta_0 = nn.Sequential(
            nn.Linear(model_params['input_size'], self.theta0_hidden_size),
            self._activation_fn(self.theta_0_hidden_actvation),
            nn.Linear(self.theta0_hidden_size, self.theta0_output_size)
        )
        ...
        self.B = nn.Parameter(torch.randn(self.M, self.z_dim, self.x_dim))
        self.C = nn.Parameter(torch.randn(self.M, self.z_dim, self.w_dim))

        self.sigma_w = nn.Parameter(torch.eye(self.w_dim))
        self.mu_w = torch.zeros((self.w_dim))
        ...
        self.f_theta_alpha = nn.Sequential(
            nn.Linear(self.z_dim + self.w_dim, self.M),
            nn.Softmax(dim=-1)
        )
        ...
        self.sigma_x = torch.eye(self.x_dim) * self.sigma_x_scalar
        ...
        self.e_xu_output_size = model_params.get('e_xu_output_size', self.h_dim) # usually 64 then
        ...
        self.e_xu = nn.Sequential(
            nn.Linear(self.x_dim + self.z_dim, self.e_xu_hidden_size),
    ...
    def forward(self, x, u):
        ...
        h = self.e_ht(xu)  # h is of shape (batch_size, seq_len, h_dim)
        ...
        for t in range(0, u.shape[1]):
            ...
            w[:, t, :] = w_t
            z[:, t, :] = z_t
            z_t = torch.einsum('ij,ijk->ik', alpha_t, self.A) @ z_t + torch.einsum('ij,j->i', alpha_t, self.B) @ u[:, t, :] + torch.einsum('ij,j->i', alpha_t, self.C) @ w_t
